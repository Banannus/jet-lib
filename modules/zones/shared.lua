--[[
    https://github.com/overextended/ox_Jet
...
local insideZones = Jet.context == 'client' and {} --[[@as table<number, CZone>]]
local exitingZones = Jet.context == 'client' and Jet.array:new() --[[@as Array<CZone>]]
local enteringZones = Jet.context == 'client' and Jet.array:new() --[[@as Array<CZone>]]
local nearbyZones = Jet.array:new() --[[@as Array<CZone>]]
...
function Jet.Zones.poly(data)
    ...
    data.__type = 'poly'
    data.radius = Jet.array.reduce(data.polygon, function(acc, point)
...
---@return CZone
function Jet.Zones.sphere(data)
    data.id = #Zones + 1
    data.coords = convertToVector(data.coords)
...
function Jet.Zones.getAllZones() return Zones end
 
function Jet.Zones.getCurrentZones() return insideZones end

function Jet.Zones.getNearbyZones() return nearbyZones end
