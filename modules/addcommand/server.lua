---@class JetCommandParams
---@field name string
---@field help? string
---@field type? 'number' | 'playerId' | 'string' | 'longString'
---@field optional? boolean

---@class JetCommandProperties
---@field help string?
---@field params JetCommandParams[]?
---@field restricted boolean | string | string[]?

---@type JetCommandProperties[]
...
---@param properties JetCommandProperties | false
---@param cb fun(source: number, args: table, raw: string)
local function AddCommand(commandName, properties, cb)
            ...
            elseif restrictedType == 'table' then
                for j = 1, #restricted do
                    if not IsPrincipalAceAllowed(restricted[j], ace) then
                        ExecuteCommand(('add_ace %s %s allow'):format(restricted[j], ace))
                    end
                end
            end
        end
