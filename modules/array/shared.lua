...
---@return Array
function Array:new(...)
    local instance = setmetatable({}, Array)
...
function Array:from(iter)
    ...
    if iterType == 'table' then
        return Array:new(table_unpack(iter))
    end

    if iterType == 'string' then
        return Array:new(string.strsplit('', iter))
    end
...
function Array:merge(...)
    ...
    return Array:new(table_unpack(newArr))
end
...
function Array:fill(value, start, endIndex)
    local length = #self
    start = start or 1
...
function Array:includes(element, fromIndex)
...
function Array:map(cb)
    ...
    return Array:new(table_unpack(arr))
end
...
---@return T
function Array:reduce(reducer, initialValue, reverse)
    local length = #self
